...
- If an L6A cluster has:
    median wavelength < 30 km
    AND maximum amplitude > 4 K
    AND median angle (0..180 symmetry) between 80 and 100 degrees
  then ignore this cluster AND also ignore any other L6A clusters in that slice that overlap it.
  Overlap means at least 1 shared finite pixel (can change overlap_min_pixels).
...
def cluster_stats_for_filter(A: np.ndarray, L: np.ndarray, T_rad: np.ndarray) -> Dict[str, float]:
    ...
    ang_med_deg_180 = circ_median_deg_180(T_rad, mask)  # 0..180 symmetry
...
def is_bad_l6a_cluster(stats: Dict[str, float]) -> bool:
    ...
    if (not np.isfinite(lam_med)) or (not np.isfinite(amp_max)) or (not np.isfinite(ang_med)):
        return False
