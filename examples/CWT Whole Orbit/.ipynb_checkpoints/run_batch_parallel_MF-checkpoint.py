...
Example:
  python run_batch.py --l6 l6 --l7 l7 --out outputs_BATCH --nproc 6
"""
...
# One pair runner
# ============================================================
def run_one_pair(l6c_path: Path, l6a_path: Path, outdir: Path, l7_dir: Path) -> Dict[str, Any]:
...
def _worker_init(matplotlib_backend: str = "Agg") -> None:
    ...
    """
    # Avoid GUI backends in workers
    mpl.use(matplotlib_backend, force=True)

    # Reduce thread over-subscription (MKL/OPENBLAS/OMP can explode with multiprocessing)
...
def _run_pair_task(args: Tuple[str, str, str, str, str]) -> Dict[str, Any]:
    """
    args: (l6c_path, l6a_path, key, outroot, l7_dir)
    """
    l6c_s, l6a_s, key, outroot_s, l7_s = args
    l6c_path = Path(l6c_s)
    l6a_path = Path(l6a_s)
    ...
    try:
        res = run_one_pair(l6c_path, l6a_path, pair_outdir, l7_dir=l7_dir)
        res["key"] = key
        return res
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--nproc", type=int, default=max(1, (os.cpu_count() or 2) - 1), help="Number of worker processes.")
...
def main() -> int:
    ...
    tasks: List[Tuple[str, str, str, str, str]] = [
        (str(l6c_path), str(l6a_path), key, str(outroot), str(l7_dir))
        for (l6c_path, l6a_path, key) in pairs
    ]

    # Force non-interactive backend in the parent too
    mpl.use("Agg", force=True)

    # IMPORTANT: guard for macOS / spawn behavior
    ...
    # Also write per-error tracebacks so you can inspect without scrolling
    if errors:
