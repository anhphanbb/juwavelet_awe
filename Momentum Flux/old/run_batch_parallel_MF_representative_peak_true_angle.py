...
def find_representative_cluster_point(
    A: np.ndarray,
    edge_y_min: int = 100,
    edge_y_max: int = 400,
    center_y: int = 250,
    fallback_y_min: int = 150,
    fallback_y_max: int = 350,
) -> Tuple[int, int, int]:
    """
    Pick one representative point for a cluster.

    Normally this is the true maximum-amplitude point. However, if that point
    is too close to the top/bottom edge of the slice, the next phase-speed
    analysis can be difficult. In that case, choose a better point.

    Rules:
      1. Start with the true maximum-amplitude point.
      2. If y is between edge_y_min and edge_y_max, keep it.
      3. If y < edge_y_min or y > edge_y_max, find another local maximum
         that is closer to center_y. If multiple local maxima work, choose the
         one closest to center_y; if tied, choose the highest amplitude.
      4. If no better local maximum exists, choose a finite cluster point
         closest to the safe band [fallback_y_min, fallback_y_max]. If tied,
         choose the highest amplitude.

    Returns:
      yy, xx, method_code

    method_code:
      0 = true max amplitude point was kept
      1 = replaced by a local maximum closer to center_y
      2 = fallback point closest to safe band, tie by highest amplitude
      -1 = no finite point found
    """
    ...
    yy0, xx0 = np.unravel_index(np.nanargmax(A), A.shape)
    yy0 = int(yy0)
    xx0 = int(xx0)

    # True peak is already usable.
    if edge_y_min <= yy0 <= edge_y_max:
        return yy0, xx0, 0

    current_dist = abs(yy0 - center_y)

    # Find 8-neighbor local maxima without scipy.
    # NaNs are treated as -inf so they cannot become maxima.
    Ap = np.where(finite, A, -np.inf)
    padded = np.pad(Ap, 1, mode="constant", constant_values=-np.inf)

    local_max = finite.copy()
    for dy in (-1, 0, 1):
        for dx in (-1, 0, 1):
            if dy == 0 and dx == 0:
                continue
    ...
    if ys.size > 0:
        dists = np.abs(ys - center_y)

        # Only replace the true peak if the new local maximum is closer to center_y.
        better = dists < current_dist
        ...
        if np.any(better):
            ...
            # Sort by distance to center first, then by highest amplitude.
            order = np.lexsort((-amp2, d2))
            k = int(order[0])
            return int(ys2[k]), int(xs2[k]), 1

    # Fallback: choose a finite cluster point closest to the preferred band.
    # Inside the band has distance 0. Ties are broken by highest amplitude.
    ys, xs = np.where(finite)

    dist_to_band = np.zeros_like(ys, dtype=float)
    low = ys < fallback_y_min
    high = ys > fallback_y_max
    dist_to_band[low] = fallback_y_min - ys[low]
    dist_to_band[high] = ys[high] - fallback_y_max

    amp = A[ys, xs]
    order = np.lexsort((-amp, dist_to_band))
    k = int(order[0])

    return int(ys[k]), int(xs[k]), 2
...
def create_passed_only_l6a_with_new_vars(
    ...
    with Dataset(src_l6a, "r") as src:
        ...
        with Dataset(dst_l7a, "w") as dst:
            ...
            peak_bearing_deg = np.full(n_passed, np.nan, dtype=np.float32)
            peak_true_angle_deg = np.full(n_passed, np.nan, dtype=np.float32)
            peak_true_angle_rad = np.full(n_passed, np.nan, dtype=np.float32)
            ...
            peak_selection_method = np.full(n_passed, -1, dtype=np.int32)
            ...
            for j in range(n_passed):
                ...
                # Save one representative point per cluster.
                # Usually this is the true maximum-amplitude point.
                # If that point is too close to the top/bottom edge, choose a better
                # local maximum closer to the center of the tile. If none exists,
                # fall back to a finite cluster point closest to y=150..350.
                A_peak_src = amp_passed[j]
                if np.any(np.isfinite(A_peak_src)):
                    yy, xx, method_code = find_representative_cluster_point(A_peak_src)
                    if yy >= 0 and xx >= 0:
                        global_x = x0 + int(xx)
                        ...
                        peak_selection_method[j] = int(method_code)
                        ...
                        # True propagation angle on the geographic grid.
                        # Dominant angle is relative to the image/tile x direction.
                        # Bearing rotates that angle into east-CCW geographic coordinates.
                        if yy < bearing_full_2d.shape[0] and global_x < bearing_full_2d.shape[1]:
                            bearing_here = bearing_full_2d[yy, global_x]
                            if np.isfinite(bearing_here) and np.isfinite(ang_rad_passed[j, yy, xx]):
                                true_angle_deg_here = (
                                    float(np.rad2deg(ang_rad_passed[j, yy, xx]))
                                    + float(bearing_here)
                                ) % 360.0

                                peak_bearing_deg[j] = np.float32(bearing_here)
                                peak_true_angle_deg[j] = np.float32(true_angle_deg_here)
                                peak_true_angle_rad[j] = np.float32(np.deg2rad(true_angle_deg_here))
            ...
            _write_1d("PeakAmplitude_DominantAngle_rad", peak_angle_rad, "Dominant angle at representative amplitude point", "radian")
            _write_1d("PeakAmplitude_DominantAngle_deg", peak_angle_deg, "Dominant angle at representative amplitude point, 0 to 180 symmetry", "degree")
            _write_1d("PeakAmplitude_Bearing_deg", peak_bearing_deg, "Bearing angle at representative amplitude point, 0=East and CCW-positive", "degree")
            _write_1d("PeakAmplitude_TrueAngle_deg", peak_true_angle_deg, "True angle at representative amplitude point: dominant angle plus bearing, modulo 360", "degree")
            _write_1d("PeakAmplitude_TrueAngle_rad", peak_true_angle_rad, "True angle at representative amplitude point: dominant angle plus bearing, modulo 2*pi", "radian")
            _write_1d("PeakAmplitude_y_index", peak_y_index, "Tile y index of representative amplitude point", "pixel")
            ...
            _write_1d("PeakAmplitude_selection_method", peak_selection_method, "Representative point selection method: 0=true peak OK, 1=local maximum closer to y=250, 2=fallback closest to y band 150-350", "1")
...
def export_peak_cluster_summary_csv(nc_path: Path, csv_path: Path) -> None:
    ...
    with Dataset(nc_path, "r") as nc:
        ...
        for j in range(n):
            rows.append({
                ...
                "peak_selection_method": int(read_1d("PeakAmplitude_selection_method", -1)[j]),
                ...
                "bearing_at_peak_deg": float(read_1d("PeakAmplitude_Bearing_deg")[j]),
                "true_angle_at_peak_deg": float(read_1d("PeakAmplitude_TrueAngle_deg")[j]),
                "true_angle_at_peak_rad": float(read_1d("PeakAmplitude_TrueAngle_rad")[j]),
            })
    ...
    fieldnames = [
        ...
        "bearing_at_peak_deg",
        "true_angle_at_peak_deg",
        "true_angle_at_peak_rad",
    ]
