...
def find_representative_cluster_point(
    A: np.ndarray,
    preferred_y_min: int = 75,
    preferred_y_max: int = 225,
) -> Tuple[int, int, int]:
    """
    Pick one representative point for a cluster.

    Rules:
      1. If the true maximum-amplitude point is inside
         [preferred_y_min, preferred_y_max], keep it.
      2. If the true maximum is outside that band, replace it with the
         maximum-amplitude point inside [preferred_y_min, preferred_y_max].
      3. If the cluster has no finite point inside that band, choose the
         finite point closest to the band. If tied, choose the highest
         amplitude.
    ...
    method_code:
      ...
      1 = replaced by maximum amplitude point inside y=75..225
      2 = fallback point closest to y=75..22, tie by highest amplitude
    ...
    # True peak is already in the preferred y band.
    if preferred_y_min <= yy0 <= preferred_y_max:
        return yy0, xx0, 0

    # First replacement: maximum amplitude inside the preferred y band.
    y_indices = np.arange(A.shape[0])[:, None]
    inside_band = finite & (y_indices >= preferred_y_min) & (y_indices <= preferred_y_max)

    if np.any(inside_band):
        A_inside = np.where(inside_band, A, -np.inf)
        yy, xx = np.unravel_index(np.argmax(A_inside), A_inside.shape)
        return int(yy), int(xx), 1

    # Fallback: choose a finite cluster point closest to the preferred band.
    # Inside the band would have distance 0, but this branch only happens
    # when there are no finite points inside the band. Ties are broken by
    # highest amplitude.
    ys, xs = np.where(finite)

    dist_to_band = np.zeros_like(ys, dtype=float)
    low = ys < preferred_y_min
    high = ys > preferred_y_max
    dist_to_band[low] = preferred_y_min - ys[low]
    dist_to_band[high] = ys[high] - preferred_y_max
...
def create_passed_only_l6a_with_new_vars(
    ...
    with Dataset(src_l6a, "r") as src:
        ...
        with Dataset(dst_l7a, "w") as dst:
            ...
            _write_1d("PeakAmplitude_selection_method", peak_selection_method, "Representative point selection method: 0=true peak OK, 1=max amplitude inside y band 75-225, 2=fallback closest to y band 75-225", "1")
