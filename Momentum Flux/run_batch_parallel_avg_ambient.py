...
Added outputs in this version:
- AverageWavelength: power-weighted average wavelength map, saved in addition to DominantWavelength
- AverageAngle: power-weighted axial/circular average angle map, saved in addition to Angle
- AmbientTemperature for L3A, or AmbientRadiance for L3C: original flattened field minus reconstructed wavy field
"""
...
def cluster_weighted_mean_maps(
    cwt: dict,
    iwave: np.ndarray,
    kcl: int,
    amp_mask_xy: np.ndarray | None = None,
    ...
    """
    Return per-pixel weighted-mean wavelength and weighted-mean angle for one cluster.

    Weight = wavelet power for coefficients belonging to this cluster.

    Wavelength is calculated from wavelength_x and wavelength_y the same way as
    the dominant wavelength map:
        lambda = 1 / sqrt((1/lambda_x)^2 + (1/lambda_y)^2)

    Angle is treated as an orientation with 180-degree symmetry, so the average
    uses an axial/circular mean:
        theta_mean = 0.5 * atan2(sum(w sin(2 theta)), sum(w cos(2 theta)))
    and is wrapped back to 0..pi.
    """
    dec = cwt["decomposition"]
    power0 = np.abs(dec) ** 2
    mask4d = (iwave == kcl)
    ...
    weights = np.where(mask2, power2, 0.0)
    wsum = np.sum(weights, axis=0)
    valid = np.isfinite(wsum) & (wsum > 0)

    lam_mean = np.full(nx * ny, np.nan, dtype=float)
    theta_mean = np.full(nx * ny, np.nan, dtype=float)

    if np.any(valid):
        lamx = np.asarray(cwt["wavelength_x"], dtype=float)  # (ns, nt)
        lamy = np.asarray(cwt["wavelength_y"], dtype=float)  # (ns, nt)
        ...
        lam_grid = np.full_like(inv2, np.nan, dtype=float)
        ok_lam = inv2 > 0
        lam_grid[ok_lam] = 1.0 / np.sqrt(inv2[ok_lam])
        lam_flat = lam_grid.reshape(ns * nt)

        good_lam_weights = weights * np.isfinite(lam_flat)[:, None]
        good_lam_sum = np.sum(good_lam_weights, axis=0)
        ok = valid & (good_lam_sum > 0)
        if np.any(ok):
            lam_mean[ok] = np.sum(good_lam_weights[:, ok] * lam_flat[:, None], axis=0) / good_lam_sum[ok]

        theta = np.asarray(cwt["theta"], dtype=float)  # (nt,)
        theta_grid = np.broadcast_to(theta[None, :], (ns, nt)).reshape(ns * nt)

        sin2 = np.sin(2.0 * theta_grid)
        cos2 = np.cos(2.0 * theta_grid)

        S = np.sum(weights[:, valid] * sin2[:, None], axis=0)
        C = np.sum(weights[:, valid] * cos2[:, None], axis=0)
        th = 0.5 * np.arctan2(S, C)
        th = np.mod(th, np.pi)
        theta_mean[valid] = th

    lam_mean_xy = lam_mean.reshape(nx, ny).T
    theta_mean_xy = theta_mean.reshape(nx, ny).T

    if amp_mask_xy is not None:
        bad = ~np.isfinite(amp_mask_xy)
        lam_mean_xy[bad] = np.nan
        theta_mean_xy[bad] = np.nan

    return lam_mean_xy, theta_mean_xy
...
def copy_lxc_to_l6x_with_clusters(
    ...
    with Dataset(lxc_path, "r") as src, Dataset(l6x_path, "w", format="NETCDF4") as dst:
        ...
        ambient_var_name = "AmbientTemperature" if product_type == "l3a" else "AmbientRadiance"
        v_ambient = dst.createVariable(
            ambient_var_name,
            "f4",
            ("cluster", "y_cluster", "x_cluster"),
            zlib=True,
        ...
        v_avg_lam.units = "km"
        v_avg_ang.units = "rad"
        v_avg_lam.long_name = "Power-weighted average wavelength for each pixel within a cluster"
        v_avg_ang.long_name = "Power-weighted axial average angle for each pixel within a cluster"
        v_ambient.units = "K" if product_type == "l3a" else "kR"
        v_ambient.long_name = "Original flattened field minus reconstructed wavy field"
        ...
        avg_lam_stack = np.full((ncl, y_dim_w, x_dim_w), np.nan, dtype=np.float32)
        avg_ang_stack = np.full((ncl, y_dim_w, x_dim_w), np.nan, dtype=np.float32)
        ambient_stack = np.full((ncl, y_dim_w, x_dim_w), np.nan, dtype=np.float32)
        ...
        for i, c in enumerate(clusters):
            ...
            AL = _unbin_repeat_xy_local(c["avg_lam_xy"], fup)
            AT = _unbin_repeat_xy_local(c["avg_theta_xy"], fup)
            AMB = _unbin_repeat_xy_local(c["ambient_xy"], fup)

            amp_stack[i] = _force_shape(A)
            lam_stack[i] = _force_shape(L)
            ang_stack[i] = _force_shape(T)
            avg_lam_stack[i] = _force_shape(AL)
            avg_ang_stack[i] = _force_shape(AT)
            ambient_stack[i] = _force_shape(AMB)
        ...
        v_lam[:] = lam_stack
        v_ang[:] = ang_stack
        v_avg_lam[:] = avg_lam_stack
        v_avg_ang[:] = avg_ang_stack
        v_ambient[:] = ambient_stack
...
def run_pipeline_for_slice(
    ...
    if cluster_ids.size == 0:
        ...
        ambient_core_xy = crop_pad_core(ambient_xy)
    ...
    for kcl in cluster_ids.astype(int):
        ...
        avg_lam_xy, avg_theta_xy = cluster_weighted_mean_maps(cwt, iwave, kcl, amp_mask_xy=A_xy)
        ...
        avg_lam_core = crop_pad_core(avg_lam_xy)
        avg_th_core = crop_pad_core(avg_theta_xy)
        ambient_core = crop_pad_core(ambient_xy)
        ...
        cluster_records.append(
            dict(
                ...
                lam_xy=lam_core,
                theta_xy=th_core,
                avg_lam_xy=avg_lam_core,
                avg_theta_xy=avg_th_core,
                ambient_xy=ambient_core,
                rec_xy=rec_core,
            )
        )
