...
#   navgem_2024075T0538_01770.nc
NAVGEM_FOLDER = Path("D:/navgem")
...
# Use TARGET_SLICES = None to process all slices.
# Use TARGET_CLUSTERS_IN_SLICE = None to process all clusters in each slice.
TARGET_SLICES = None
TARGET_CLUSTERS_IN_SLICE = range(15)
...
# Base folder for cluster/debug plots. Each orbit gets its own subfolder.
PLOT_OUT_ROOT_DIR = Path("plots_l7a_cluster_debug")
PLOT_OUT_BASE_DIR = PLOT_OUT_ROOT_DIR
...
# SSE threshold for phase fit quality control.
# In plots, SSE > this value is highlighted in red.
# In L8A slice/top sums, clusters with finite SSE > this value are excluded.
SSE_BAD_THRESHOLD = 0.5
...
# ============================================================
# These are set automatically for each orbit inside main()
# ============================================================
NC_PATH = None
NAVGEM_NC_PATH = None
L1A_REMAP_NC_PATH = None
PHASE_SPEED_BASE_OUT_DIR = None
...
def calculate_phase_fit_sse_from_summary(phase_summary_csv):
    """
    Recalculate the cosine-fit SSE from the phase-speed output folder.

    Returns NaN if the crest-relative-amplitude CSV is not available or the
    fit cannot be recomputed.
    """
...
def save_cluster_plot(
    *,
    ...
    if np.isfinite(phase_fit_sse) and phase_fit_sse > SSE_BAD_THRESHOLD:
        ax_info.text(
            0.02,
            0.02,
            f"SSE > {SSE_BAD_THRESHOLD}: excluded from NewMF slice/top sums",
            transform=ax_info.transAxes,
            va="bottom",
            ha="left",
            family="monospace",
...
def process_one_cluster(nc, slices_all, TARGET_SLICE, TARGET_CLUSTER_IN_SLICE, navgem_data):
    ...
    if CALCULATE_PHASE_SPEED:
        if not np.isfinite(lat_saved) or not np.isfinite(lon_saved):
            ...
            phase_fit_sse = calculate_phase_fit_sse_from_summary(phase_summary_csv)
    ...
    if SAVE_L8A_FILE:
        L8A_CLUSTER_OUTPUTS.append({
            ...
            "PhaseFitSSE_cluster": float(phase_fit_sse),
        })
    ...
    return {
        ...
        "phase_fit_sse": float(phase_fit_sse),
        "excluded_from_mf_sums_sse_gt_threshold": bool(np.isfinite(phase_fit_sse) and phase_fit_sse > SSE_BAD_THRESHOLD),
        "phase_summary_csv": phase_summary_csv,
...
def save_l8a_with_new_variables(src_l7a_path, dst_l8a_path, cluster_outputs):
    ...
    with Dataset(dst_l8a_path, "r+") as nc:
        ...
        scalar_defs = [
            ...
            ("PhaseFitSSE_cluster", "Cosine-fit SSE from crest relative amplitude evolution", "1"),
        ]
        ...
        if "SlicesNo" in nc.variables:
            ...
            processed_idx_all = np.array(
                [int(item["global_cluster_index"]) for item in cluster_outputs],
                dtype=int,
            )
            processed_idx_all = processed_idx_all[(processed_idx_all >= 0) & (processed_idx_all < n_cluster)]

            # For NewMF_slice and NewMF_top, exclude bad phase fits.
            # Only finite SSE > threshold is excluded. If SSE is NaN, the cluster is kept,
            # because it may mean the SSE file was unavailable rather than the fit was bad.
            phase_sse_all = _make_full_scalar_array("PhaseFitSSE_cluster")
            bad_sse = np.isfinite(phase_sse_all) & (phase_sse_all > SSE_BAD_THRESHOLD)
            processed_idx = processed_idx_all[~bad_sse[processed_idx_all]]
            excluded_idx = processed_idx_all[bad_sse[processed_idx_all]]
            if excluded_idx.size > 0:
                print(
                    f"Excluding {excluded_idx.size} clusters from NewMF slice/top sums "
                    f"because PhaseFitSSE_cluster > {SSE_BAD_THRESHOLD}: "
                    f"{excluded_idx.tolist()}"
                )
            ...
            if processed_idx.size > 0:
                ...
                _write_or_update("NewMF_slice", new_mf_slice, slice_dims,
                                 "Per-slice sum of NewMF for processed clusters with SSE <= threshold", "m2 s-2")
                _write_or_update("NewMF_u_slice", new_mf_u_slice, slice_dims,
                                 "Per-slice sum of signed zonal NewMF for processed clusters with SSE <= threshold", "m2 s-2")
                _write_or_update("NewMF_v_slice", new_mf_v_slice, slice_dims,
                                 "Per-slice sum of signed meridional NewMF for processed clusters with SSE <= threshold", "m2 s-2")
                ...
                if top_template_name is not None:
                    ...
                    if len(top_shape) == 3:
                        ...
                        _write_or_update("NewMF_top", new_mf_top, top_dims,
                                         "Stitched NewMF sum on top grid for processed clusters with SSE <= threshold", "m2 s-2")
                        _write_or_update("NewMF_u_top", new_mf_u_top, top_dims,
                                         "Stitched signed zonal NewMF sum on top grid for processed clusters with SSE <= threshold", "m2 s-2")
                        _write_or_update("NewMF_v_top", new_mf_v_top, top_dims,
                                         "Stitched signed meridional NewMF sum on top grid for processed clusters with SSE <= threshold", "m2 s-2")
        ...
        nc.setncattr(
            "l8a_newmf_sse_threshold",
            float(SSE_BAD_THRESHOLD),
        )
        nc.setncattr(
            "l8a_newmf_sum_rule",
            "NewMF_slice and NewMF_top exclude clusters with finite PhaseFitSSE_cluster greater than l8a_newmf_sse_threshold.",
        )
...
# Main
# ============================================================
def parse_l7a_filename(nc_path):
    """
    Parse an L7A file name.

    Example:
        awe_l7a_tmp_2024075T0538_01770_v23.nc

    Returns:
        orbit_tag = 2024075T0538_01770
        version   = v23
        orbit_id  = 01770
    """
    ...
    m = re.match(r"^awe_l7a_tmp_(.+)_(v\d+)$", stem)
    if not m:
        raise ValueError(
            f"Could not parse L7A filename: {nc_path.name}\n"
            "Expected something like awe_l7a_tmp_2024075T0538_01770_v23.nc"
        )

    orbit_tag = m.group(1)
    version = m.group(2)
    orbit_id = orbit_tag.split("_")[-1]

    return orbit_tag, version, orbit_id


def build_matching_paths(nc_path):
    """
    Build the matching NAVGEM and L1A/remap85 paths from one L7A path.
    """
    orbit_tag, version, orbit_id = parse_l7a_filename(nc_path)

    navgem_path = NAVGEM_FOLDER / f"navgem_{orbit_tag}.nc"

    l1a_remap_path = L1A_REMAP_FOLDER / (
        f"awe_l1a_q20_{orbit_tag}_{version}_remap85.nc"
    )

    return orbit_tag, version, orbit_id, navgem_path, l1a_remap_path


def process_one_l7a_file(nc_path):
    """
    Process one L7A file:
        - find matching NAVGEM file
        - find matching L1A remap85 file
        - run selected/all slices and clusters
        - save combined summary CSV
        - save copied L8A with old + new variables
    """
    global NC_PATH
    global NAVGEM_NC_PATH
    global L1A_REMAP_NC_PATH
    global PHASE_SPEED_BASE_OUT_DIR
    global COMBINED_SUMMARY_CSV
    global L8A_OUT_PATH
    global L8A_CLUSTER_OUTPUTS
    global PLOT_OUT_BASE_DIR

    NC_PATH = Path(nc_path)

    orbit_tag, version, orbit_id, navgem_path, l1a_remap_path = build_matching_paths(NC_PATH)

    NAVGEM_NC_PATH = navgem_path
    L1A_REMAP_NC_PATH = l1a_remap_path

    # Separate output folders by orbit so outputs do not get mixed.
    PHASE_SPEED_BASE_OUT_DIR = Path("AWE") / orbit_id / "remap85_from_l7a"
    COMBINED_SUMMARY_CSV = (
        PHASE_SPEED_BASE_OUT_DIR
        / "selected_slices_clusters_phase_speed_summary.csv"
    )

    PLOT_OUT_BASE_DIR = PLOT_OUT_ROOT_DIR / orbit_id
    ...
    # Reset for each orbit. Very important, otherwise clusters from older
    # orbits can accidentally get written into the next L8A file. Gross.
    L8A_CLUSTER_OUTPUTS = []

    print("=" * 78)
    print(f"File                    : {NC_PATH}")
    print(f"Orbit tag               : {orbit_tag}")
    print(f"Orbit ID                : {orbit_id}")
    print(f"Version                 : {version}")
    print(f"NAVGEM file             : {NAVGEM_NC_PATH}")
    print(f"L1A remap file          : {L1A_REMAP_NC_PATH}")
    ...
    if not NC_PATH.exists():
        print(f"Missing L7A file. Skipping: {NC_PATH}")
        return {
            "file": str(NC_PATH),
            "orbit_tag": orbit_tag,
            "status": "skipped_missing_l7a",
        }

    if not NAVGEM_NC_PATH.exists():
        print(f"Missing NAVGEM file. Skipping this orbit: {NAVGEM_NC_PATH}")
        return {
            "file": str(NC_PATH),
            "orbit_tag": orbit_tag,
            "status": "skipped_missing_navgem",
        }

    if CALCULATE_PHASE_SPEED and not L1A_REMAP_NC_PATH.exists():
        print(f"Missing L1A remap85 file. Skipping this orbit: {L1A_REMAP_NC_PATH}")
        return {
            "file": str(NC_PATH),
            "orbit_tag": orbit_tag,
            "status": "skipped_missing_l1a_remap85",
        }

    # Read NAVGEM once per orbit. Each cluster extracts the correct slice from these raw 2D fields.
        ...
        for target_slice in slices_to_run:
            ...
            for target_cluster in clusters_to_run:
                try:
                    ...
                    row["file"] = str(NC_PATH)
                    row["orbit_tag"] = orbit_tag
                    row["orbit_id"] = orbit_id
                    row["version"] = version
                    results.append(row)

                except Exception as exc:
                    print(
                        f"  ERROR for orbit {orbit_tag}, "
                        f"slice {target_slice}, cluster {target_cluster}: {exc}"
                    )
                    traceback.print_exc()
                    results.append({
                        "file": str(NC_PATH),
    ...
    return {
        ...
        "l8a_file": str(l8a_saved),
        "n_rows": len(results),
    }


def save_all_orbits_summary(rows, out_csv):
    out_csv = Path(out_csv)
    out_csv.parent.mkdir(parents=True, exist_ok=True)

    if not rows:
        print("No orbit-level rows to save.")
        return
    ...
    print(f"Saved all-orbits summary: {out_csv}")


# ============================================================
# Parallel settings
# ============================================================
# Parallelize by orbit/L7A file. This is safer than parallelizing by cluster,
# because each orbit writes its own CSV, plot folders, and L8A file.
USE_PARALLEL = True
...
def main():
    l7_files = sorted(L7_FOLDER.glob(L7_GLOB))

    print("=" * 78)
    print(f"L7 folder       : {L7_FOLDER}")
    print(f"L7 glob         : {L7_GLOB}")
    print(f"NAVGEM folder   : {NAVGEM_FOLDER}")
    print(f"L1A remap folder: {L1A_REMAP_FOLDER}")
    print(f"Found L7A files : {len(l7_files)}")
    print(f"Use parallel    : {USE_PARALLEL}")
    print(f"Workers         : {N_WORKERS}")
    print("=" * 78)

    if not l7_files:
        print("No L7A files found. Check L7_FOLDER and L7_GLOB.")
        return

    orbit_rows = []

    if USE_PARALLEL and len(l7_files) > 1:
        n_workers = max(1, min(int(N_WORKERS), len(l7_files)))

        with ProcessPoolExecutor(max_workers=n_workers) as executor:
            futures = {
                executor.submit(process_one_l7a_file, nc_path): nc_path
                for nc_path in l7_files
            }
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    print(f"Finished parallel job: {nc_path}")

                except Exception as exc:
                    print(f"ERROR while processing {nc_path}: {exc}")
                    traceback.print_exc()
                    orbit_rows.append({
                        "file": str(nc_path),
                        "status": "error",
        ...
        for nc_path in l7_files:
            try:
                row = process_one_l7a_file(nc_path)
                orbit_rows.append(row)
    ...
    save_all_orbits_summary(
        orbit_rows,
        Path("AWE") / "all_orbits_phase_speed_l8a_summary.csv",
    )

    print("All orbits done.")
