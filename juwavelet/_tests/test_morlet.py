import numpy as np
import pytest

import juwavelet.morlet as wavelet


SST = np.asarray([
    -0.15, -0.30, -0.14, -0.41, -0.46, -0.66, -0.50, -0.80, -0.95,
    -0.72, -0.31, -0.71, -1.04, -0.77, -0.86, -0.84, -0.41, -0.49,
    -0.48, -0.72, -1.21, -0.80, 0.16, 0.46, 0.40, 1.00, 2.17,
    2.50, 2.34, 0.80, 0.14, -0.06, -0.34, -0.71, -0.34, -0.73,
    -0.48, -0.11, 0.22, 0.51, 0.51, 0.25, -0.10, -0.33, -0.42,
    -0.23, -0.53, -0.44, -0.30, 0.15, 0.09, 0.19, -0.06, 0.25,
    0.30, 0.81, 0.26, 0.10, 0.34, 1.01, -0.31, -0.90, -0.73,
    -0.92, -0.73, -0.31, -0.03, 0.12, 0.37, 0.82, 1.22, 1.83,
    1.60, 0.34, -0.72, -0.87, -0.85, -0.40, -0.39, -0.65, 0.07,
    0.67, 0.39, 0.03, -0.17, -0.76, -0.87, -1.36, -1.10, -0.99,
    -0.78, -0.93, -0.87, -0.44, -0.34, -0.50, -0.39, -0.04, 0.42,
    0.62, 0.17, 0.23, 1.03, 1.54, 1.09, 0.01, 0.12, -0.27,
    -0.47, -0.41, -0.37, -0.36, -0.39, 0.43, 1.05, 1.58, 1.25,
    0.86, 0.60, 0.21, 0.19, -0.23, -0.29, 0.18, 0.12, 0.71,
    1.42, 1.59, 0.93, -0.25, -0.66, -0.95, -0.47, 0.06, 0.70,
    0.81, 0.78, 1.43, 1.22, 1.05, 0.44, -0.35, -0.67, -0.84,
    -0.66, -0.45, -0.12, -0.20, -0.16, -0.47, -0.52, -0.79, -0.80,
    -0.62, -0.86, -1.29, -1.04, -1.05, -0.75, -0.81, -0.90, -0.25,
    0.62, 1.22, 0.96, 0.21, -0.11, -0.25, -0.24, -0.43, 0.23,
    0.67, 0.78, 0.41, 0.98, 1.28, 1.45, 1.02, 0.03, -0.59,
    -1.34, -0.99, -1.49, -1.74, -1.33, -0.55, -0.51, -0.36, -0.99,
    0.32, 1.04, 1.41, 0.99, 0.66, 0.50, 0.22, 0.71, -0.16,
    0.38, 0.00, -1.11, -1.04, 0.05, -0.64, -0.34, -0.50, -1.85,
    -0.94, -0.78, 0.29, 0.27, 0.69, -0.06, -0.83, -0.80, -1.02,
    -0.96, -0.09, 0.62, 0.87, 1.03, 0.70, -0.10, -0.31, 0.04,
    -0.46, 0.04, 0.24, -0.08, -0.28, 0.06, 0.05, -0.31, 0.11,
    0.27, 0.26, 0.04, 0.12, 1.11, 1.53, 1.23, 0.17, -0.18,
    -0.56, 0.05, 0.41, 0.22, 0.04, -0.19, -0.46, -0.65, -1.06,
    -0.54, 0.14, 0.25, -0.21, -0.73, -0.43, 0.48, 0.26, 0.05,
    0.11, -0.27, -0.08, -0.10, 0.29, -0.15, -0.28, -0.55, -0.44,
    -1.40, -0.55, -0.69, 0.58, 0.37, 0.42, 1.83, 1.23, 0.65,
    0.41, 1.03, 0.64, -0.07, 0.98, 0.36, -0.30, -1.33, -1.39,
    -0.94, 0.34, -0.00, -0.15, 0.06, 0.39, 0.36, -0.49, -0.53,
    0.35, 0.07, -0.24, 0.20, -0.22, -0.68, -0.44, 0.02, -0.22,
    -0.30, -0.59, 0.10, -0.02, -0.27, -0.60, -0.48, -0.37, -0.53,
    -1.35, -1.22, -0.99, -0.34, -0.79, -0.24, 0.02, 0.69, 0.78,
    0.17, -0.17, -0.29, -0.27, 0.31, 0.44, 0.38, 0.24, -0.13,
    -0.89, -0.76, -0.71, -0.37, -0.59, -0.63, -1.47, -0.40, -0.18,
    -0.37, -0.43, -0.06, 0.61, 1.33, 1.19, 1.13, 0.31, 0.14,
    0.03, 0.21, 0.15, -0.22, -0.02, 0.03, -0.17, 0.12, -0.35,
    -0.06, 0.38, -0.45, -0.32, -0.33, -0.49, -0.14, -0.56, -0.18,
    0.46, 1.09, 1.04, 0.23, -0.99, -0.59, -0.92, -0.28, 0.52,
    1.31, 1.45, 0.61, -0.11, -0.18, -0.39, -0.39, -0.36, -0.50,
    -0.81, -1.10, -0.29, 0.57, 0.68, 0.78, 0.78, 0.63, 0.98,
    0.49, -0.42, -1.34, -1.20, -1.18, -0.65, -0.42, -0.97, -0.28,
    0.77, 1.77, 2.22, 1.05, -0.67, -0.99, -1.52, -1.17, -0.22,
    -0.04, -0.45, -0.46, -0.75, -0.70, -1.38, -1.15, -0.01, 0.97,
    1.10, 0.68, -0.02, -0.04, 0.47, 0.30, -0.55, -0.51, -0.09,
    -0.01, 0.34, 0.61, 0.58, 0.33, 0.38, 0.10, 0.18, -0.30,
    -0.06, -0.28, 0.12, 0.58, 0.89, 0.93, 2.39, 2.44, 1.92,
    0.64, -0.24, 0.27, -0.13, -0.16, -0.54, -0.13, -0.37, -0.78,
    -0.22, 0.03, 0.25, 0.31, 1.03, 1.10, 1.05, 1.11, 1.28,
    0.57, -0.55, -1.16, -0.99, -0.38, 0.01, -0.29, 0.09, 0.46,
    0.57, 0.24, 0.39, 0.49, 0.86, 0.51, 0.95, 1.25, 1.33,
    -0.00, 0.34, 0.66, 1.11, 0.34, 0.48, 0.56, 0.39, -0.17,
    1.04, 0.77, 0.12, -0.35, -0.22, 0.08, -0.08, -0.18, -0.06])


@pytest.mark.parametrize("scale", [20, 50])
@pytest.mark.parametrize("param", [4, 6, 8, 10])
@pytest.mark.parametrize("normed", [False, True])
def test_morlet1d(param, scale, normed):
    xs = np.arange(0., 1200., 0.1)
    dx = xs[1] - xs[0]
    n_x = len(xs)

    morletf = wavelet.morlet1d_fourier(n_x, dx, scale, param, normed=normed)
    morletr = wavelet.morlet1d(n_x, dx, scale, param, normed=normed)
    morletr_p = wavelet._morlet1d(n_x, dx, scale, param, normed=normed)
    morletf_r = np.fft.ifft(morletf)
    if normed:
        print((morletf * morletf.conj()).sum() / (2 * len(xs)))
        print((morletr * morletr.conj()).sum())
        print(np.trapz(morletf * morletf.conj(), dx=dx) / (2 * len(xs)))
        print(np.trapz(morletr * morletr.conj(), dx=dx))
        # check proper norm of morlet wavelet
        assert abs(np.trapz(morletr * morletr.conj(), dx=dx) - 1.0) < 0.05
        # check proper norm of morlet wavelet (fourier space)
        assert abs(np.trapz(morletf * morletf.conj(), dx=dx) / (len(xs)) - 1) < 0.05
        assert abs(np.trapz(morletf_r * morletf_r.conj(), dx=dx) - 1) < 0.05

    print("r ", morletr.mean())
    print("rp", morletr_p.mean())
    # check on agreement between fourier and spatial implementation
    assert np.allclose(morletr, morletr_p)


@pytest.mark.parametrize("scale", [30, 50])
@pytest.mark.parametrize("param", [4, 6, 8, 10])
@pytest.mark.parametrize("aspect", [10, 20])
@pytest.mark.parametrize("normed", [False, True])
@pytest.mark.parametrize("theta", [0, np.pi / 4])
def test_morlet2d_real(param, scale, aspect, normed, theta):
    xs = np.arange(0., 1200., 1)
    ys = np.arange(0, 80, 0.2)
    dx = xs[1] - xs[0]
    dy = ys[1] - ys[0]
    n_x = len(xs)
    n_y = len(ys)

    morletr = wavelet.morlet2d(
        n_x, n_y, dx, dy * aspect, scale, theta, param,
        normed=normed)
    morletf = wavelet.morlet2d_fourier(
        n_x, n_y, dx, dy * aspect, scale, theta, param,
        normed=normed)
    morletr_p = wavelet._morlet2d(
        n_x, n_y, dx, dy * aspect, scale, theta, param,
        normed=normed)
    morletf_r = np.fft.ifft2(morletf)
    if normed:
        print((morletr * morletr.conj()).sum())
        assert abs(dx * dy * aspect * (morletr * morletr.conj()).sum() - 1.0) < 0.05
        assert abs(dx * dy * aspect * (morletr_p * morletr_p.conj()).sum() - 1.0) < 0.05
        assert abs(dx * dy * aspect * (morletf_r * morletf_r.conj()).sum() - 1.0) < 0.05
    # import matplotlib.pyplot as plt
    # plt.subplot(3, 1, 1)
    # plt.pcolormesh(morlet.real)
    # plt.colorbar()
    # plt.subplot(3, 1, 2)
    # plt.pcolormesh(morlet_p.real)
    # plt.colorbar()
    # plt.subplot(3, 1, 3)
    # plt.pcolormesh((morlet-morlet_p).real)
    # plt.colorbar()
    # plt.show()
    assert np.allclose(morletr, morletr_p)


@pytest.mark.parametrize("dx", [0.1, 0.08])
@pytest.mark.parametrize("scale", [1, 1.5])
@pytest.mark.parametrize("param", [4, 2 * np.pi])
@pytest.mark.parametrize("aspect", [1, 2])
@pytest.mark.parametrize("normed", [True, False])
@pytest.mark.parametrize("theta", [0, np.pi / 8])
@pytest.mark.parametrize("phi", [0, np.pi / 8])
def test_morlet3d_real(dx, param, scale, aspect, normed, theta, phi):
    print(dx, param, scale, aspect, normed, theta, phi)
    xs = np.arange(0., 19, dx)
    ys = np.arange(0., 18, 0.1)
    zs = np.arange(0, 26, 0.1)
    dx = xs[1] - xs[0]
    dy = ys[1] - ys[0]
    dz = zs[1] - zs[0]
    n_x = len(xs)
    n_y = len(ys)
    n_z = len(zs)

    morlet = wavelet.morlet3d(
        n_x, n_y, n_z, dx, dy, dz * aspect, scale,
        theta, phi, param, normed=normed)
    morlet_p = wavelet._morlet3d(
        n_x, n_y, n_z, dx, dy, dz * aspect, scale,
        theta, phi, param, normed=normed)
    # import matplotlib.pyplot as plt
    # plt.plot(zs, col)
    # plt.show()
    print(morlet.shape)
    print(morlet_p.shape)
    print(xs.shape)
    print(ys.shape)
    print(zs.shape)
    print(morlet[morlet.shape[0] // 2].real.shape)
    print(morlet_p[morlet.shape[0] // 2].real.shape)
    morlet_norm = (morlet * morlet.conj()).sum()
    morlet_p_norm = (morlet_p * morlet_p.conj()).sum()
    print(morlet_norm)
    print(morlet_p_norm)
    morlet_p_norm = (morlet_p * morlet_p.conj()).sum()
    if normed:
        print(dx * dy * dz * aspect * morlet_norm)
        print(dx * dy * dz * aspect * morlet_p_norm)
        assert abs(dx * dy * dz * aspect * morlet_norm - 1.0) < 0.05
    assert np.allclose(morlet, morlet_p)
